...
> **UNIVERSAL** — Shared across ALL workbook report migrations. NEVER overwritten per workbook.
> Workbook-specific theme overrides belong in `.specify/memory/{WorkbookName}/theme-overrides.md`.
...
## Chart Type Mapping (Tableau → Power BI)
...
| Pie | pieChart |
| Square + size | treemap |
| Circle / Shape (scatter) | scatterChart |
| Text table | tableEx / matrix |
| Map (filled) | filledMap / shapeMap |
| Automatic (single measure) | card |
| Line + Circle KPI sparkline | lineChart (card-styled) |

## Naming Conventions

| Item | Convention |
|---|---|
| Page names | Same as Tableau dashboard name, sanitized to `^[\w-]+$` |
