...
- **NEVER enable both title AND header on slicers** — slicers have a built-in header that shows the field name. Set `visualContainerObjects.title.show = false` (or omit title entirely). Style the label via `objects.header[].properties.fontColor` instead.
